...
type FdProduct = {
  ...
  minDepositTermType?: { value?: string };
  maxDepositTerm?: number;
  maxDepositTermType?: { value?: string };
};
...
export default function FdProductsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fineract<FdProduct[]>({
          method: "GET",
    ...
    <AppShell>
      <header className="page-head">
        <div>
          ...
          <h1 className="page-title">Fixed deposit products</h1>
          <p className="page-sub">
            Templates for term deposits. Define rate, term limits, and pre-closure rules.
          </p>
        </div>
        <Link href="/fixed-deposit-products/new" className="btn btn-primary" style={{ textDecoration: "none" }}>
          New product
        </Link>
      </header>
      ...
      <div className="table-card">
        <div className="table-head">
          ...
          <span>GET /fixeddepositproducts</span>
        </div>
        {products === null ? (
          ...
          <div className="empty-state">
            ...
            <p style={{ marginBottom: 16 }}>
              Define your first fixed deposit product to start accepting term deposits.
            </p>
            <Link href="/fixed-deposit-products/new" className="btn btn-primary" style={{ textDecoration: "none" }}>
              Create a product
            </Link>
              ...
              {products.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="mono" style={{ color: "var(--ink-soft)" }}>
                    {p.minDepositTerm
                      ? `${p.minDepositTerm} ${p.minDepositTermType?.value ?? ""}`
                      : "—"}
                    {" – "}
                    {p.maxDepositTerm
                      ? `${p.maxDepositTerm} ${p.maxDepositTermType?.value ?? ""}`
                      : "—"}
                  </td>
                </tr>
              ))}
